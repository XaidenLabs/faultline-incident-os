...
| Iteration 6 | Separated real observation from reproducible intervention. Added scheduled GitHub, Cloudflare, and npm capture, append-only Supabase memory, change fingerprints, Realtime updates, and ingestion-health records. | Production verification: three sources captured per run; unchanged runs append three snapshots and zero duplicate observation events. | Kept. Real data makes the product operational; the synthetic lab remains necessary for safe causal proof and fair scoring. |
